...
int main(void) {
    ...
    while (1) {
       ...
       // exit command:
        ...
        //child process:
        if (pid == 0) {
            char *argv[MAX_ARGS]; //argv array that will contain the tolkens of the command.
            tokenize(buffer, argv, MAX_ARGS);
            if (argv[0] == NULL) _exit(0);
            if (redirections(argv) < 0) _exit(2);
            execvp(argv[0], argv);
            _exit(127);
        }
        ...
        if (pid == 0) {
            ...
            tokenize(buffer, argv, MAX_ARGS);
            if (argv[0] == NULL) _exit(0);
            execvp(argv[0], argv); //runs the program argv[0] with the arguments argv
